...
"""Opção 2: aproximar a condição de contorno natural com diferença central.

\begin{equation}
    \frac{du}{dx}\Big|_{x=x_i} = \frac{u_5 - u_3}{0.5} = 1
\end{equation}
"""
...
"""Veja a Eq. (28) das Notas de Aula. Note que para $i = 3$, que é  o terceiro ponto, temos que calcular $u_4$, que é justamente o valor do qual só conhecemos a derivada. A equação nesse ponto $i=4$ é dada por:

\begin{equation}
    u_5 - 1.9375u_4 + u_3 = -0.25^2\cdot1 = -0.0625
\end{equation}

Como estamos aproximando por diferença central, geramos um ponto $u_5$ fora do domínio. Por isso, temos que construir uma equação para ele também, dada por:
...
#
A[-1,-1] = (dx**2)  -  2   #coeficiente de u4
A[-1,-2] = 2               #coeficiente de u3
b[-1] = b[-1] - 0.5        #ajuste da condição de contorno na última equação
